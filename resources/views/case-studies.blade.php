<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Case Studies — Trackly</title>
  <meta name="description" content="How Trackly clients grew market share, shortened sales cycles, and tripled inbound leads.">
  <meta property="og:type" content="website">
  <meta property="og:title" content="Case Studies — Trackly">
  <meta property="og:description" content="How Trackly clients grew market share, shortened sales cycles, and tripled inbound leads.">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:ital,wght@0,400;0,500;0,600;0,700;0,800;1,700&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="css/style.css">
</head>
<body>

  <header class="site-header" id="siteHeader">
    <div class="nav">
      <a class="logo" href="{{ route('home') }}" aria-label="Trackly home">
        <img src="images/W2RMPEQ1nAhx.webp" alt="Trackly" width="114" height="32">
      </a>

      <nav class="nav-links" id="menu" aria-label="Main">
        <a href="{{ route('home') }}">Home</a>
        <a href="{{ route('about') }}">About Us</a>
        <a href="{{ route('services') }}">Services</a>
        <a href="{{ route('case-studies') }} class="active" aria-current="page"">Case Studies</a>
        <a href="{{ route('blog') }}">Blog</a>
        <a href="{{ route('contact') }}" class="menu-cta btn btn-primary btn-sm">Contact Us</a>
      </nav>

      <div class="nav-actions">
        <a class="btn btn-primary btn-sm nav-contact" href="{{ route('contact') }}">Contact Us</a>
        <button class="menu-toggle" id="menuToggle" aria-expanded="false" aria-controls="menu" aria-label="Open menu">
          <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
            <path class="bars" d="M4 7h16M4 12h16M4 17h16"/>
            <path class="cross" d="M6 6l12 12M18 6L6 18"/>
          </svg>
        </button>
      </div>
    </div>
  </header>

  <main>
    <section class="page-hero">
      <div class="container">
        <nav class="crumbs" aria-label="Breadcrumb"><a href="{{ route('home') }}">Home</a><span>/</span><span aria-current="page">Case Studies</span></nav>
        <p class="tag">Case studies</p>
        <h1>Real Results That Speak for Themselves</h1>
        <p class="page-hero-sub">A closer look at the problems our clients brought us, what we changed, and what it produced.</p>
      </div>
    </section>

    <section class="section">
      <div class="container">
        <ul class="stats reveal">
          <li><strong>30%</strong><span>Average market share gain</span></li>
          <li><strong>3.1x</strong><span>Inbound lead growth</span></li>
          <li><strong>19</strong><span>Days off the sales cycle</span></li>
          <li><strong>92%</strong><span>Forecast accuracy</span></li>
        </ul>
      </div>
    </section>

    <section class="section">
      <div class="container">
        <header class="section-head center reveal">
          <p class="tag">Selected work</p>
          <h2 class="h4">Projects and the Numbers Behind Them</h2>
        </header>
        <div class="case-list">
          <article class="case-card reveal">
            <div class="case-card-media"><img src="images/HT85WVAgxN1M.webp" alt="" loading="lazy"></div>
            <div class="case-card-body">
              <div class="case-card-meta"><span class="badge badge-blue">Market expansion</span><span class="case-client">NexGen Solutions</span></div>
              <h3 class="h5">Driving Market Expansion for NexGen Solutions</h3>
              <dl class="case-dl">
                <div><dt>Challenge</dt><dd>Entering new markets with minimal brand recognition.</dd></div>
                <div><dt>Solution</dt><dd>A targeted content strategy paired with localized advertising in each region.</dd></div>
                <div><dt>Result</dt><dd>Expanded into three new regions and grew market share by 30%.</dd></div>
              </dl>
            </div>
          </article>
          <article class="case-card reveal">
            <div class="case-card-media"><img src="images/LJqsreJLDNu42kljrvPPLIhru90_scale-down-to=1024.webp" alt="" loading="lazy"></div>
            <div class="case-card-body">
              <div class="case-card-meta"><span class="badge badge-blue">Conversion</span><span class="case-client">Brightwave Retail</span></div>
              <h3 class="h5">Rebuilding a Checkout Flow That Was Losing Half Its Carts</h3>
              <dl class="case-dl">
                <div><dt>Challenge</dt><dd>A six-step checkout with a 61% abandonment rate.</dd></div>
                <div><dt>Solution</dt><dd>Cut the flow to two steps, added saved payment details and clear delivery pricing.</dd></div>
                <div><dt>Result</dt><dd>Abandonment dropped to 28% and revenue per visitor rose 44%.</dd></div>
              </dl>
            </div>
          </article>
          <article class="case-card reveal">
            <div class="case-card-media"><img src="images/47HioZdKUziH7OEXbCHZPeKRGY_scale-down-to=1024.webp" alt="" loading="lazy"></div>
            <div class="case-card-body">
              <div class="case-card-meta"><span class="badge badge-blue">Sales enablement</span><span class="case-client">Orbit Logistics</span></div>
              <h3 class="h5">Giving a 40-Person Sales Team One Source of Truth</h3>
              <dl class="case-dl">
                <div><dt>Challenge</dt><dd>Three CRMs, no shared definition of a qualified lead.</dd></div>
                <div><dt>Solution</dt><dd>Consolidated records into a single pipeline with scoring rules the team agreed on.</dd></div>
                <div><dt>Result</dt><dd>Sales cycle shortened by 19 days and forecast accuracy reached 92%.</dd></div>
              </dl>
            </div>
          </article>
          <article class="case-card reveal">
            <div class="case-card-media"><img src="images/ONUKiJ67eCtT.webp" alt="" loading="lazy"></div>
            <div class="case-card-body">
              <div class="case-card-meta"><span class="badge badge-blue">Lead generation</span><span class="case-client">Northline Finance</span></div>
              <h3 class="h5">Turning a Quiet Blog Into the Main Acquisition Channel</h3>
              <dl class="case-dl">
                <div><dt>Challenge</dt><dd>Steady traffic that produced almost no qualified leads.</dd></div>
                <div><dt>Solution</dt><dd>Reworked content around buyer questions and added gated tools with lead scoring.</dd></div>
                <div><dt>Result</dt><dd>Inbound demo requests grew 3.1x in eight months at no extra ad spend.</dd></div>
              </dl>
            </div>
          </article>
        </div>
      </div>
    </section>
  </main>

  <div class="cta-wrap">
    <section class="cta reveal" aria-labelledby="ctaTitle">
      <svg class="cta-mark" viewBox="0 0 300 300" fill="none" aria-hidden="true">
        <g opacity="0.12">
          <path d="M299.587 161.25H246C199.2 161.25 161.25 199.19 161.25 246v53.58c73.88-5.48 132.86-64.46 138.34-138.33Z" fill="#fff"/>
          <path d="M138.75 299.58V246c0-46.8-37.94-84.75-84.75-84.75H.42c5.47 73.87 64.46 132.86 138.33 138.33Z" fill="#fff"/>
          <path d="M299.587 138.75C294.11 64.87 235.13 5.89 161.25.41V54c0 46.8 37.94 84.75 84.75 84.75h53.59Z" fill="#fff"/>
          <path d="M138.75.41C64.88 5.89 5.9 64.87.42 138.75H54c46.8 0 84.75-37.94 84.75-84.75V.41Z" fill="#fff"/>
          <path d="M101.87 150c20.8-10.39 37.74-27.34 48.13-48.13 10.39 20.8 27.34 37.74 48.13 48.13-20.8 10.39-37.74 27.34-48.13 48.13-10.39-20.8-27.34-37.74-48.13-48.13Z" fill="#fff" fill-opacity=".8"/>
        </g>
      </svg>
      <div class="cta-text">
        <h2 class="h4" id="ctaTitle">Don’t wait — transform your business with our expertise.</h2>
        <p>Ready to grow your business?</p>
      </div>
      <div class="cta-actions">
        <a class="btn btn-secondary btn-md" href="{{ route('contact') }}">Start Free Trial</a>
        <a class="btn btn-tertiary btn-md" href="{{ route('services') }}">Learn More</a>
      </div>
    </section>
  </div>

  <footer class="site-footer">
    <div class="footer-inner">
      <div class="footer-top">
        <div class="footer-about">
          <a href="{{ route('home') }}" class="logo"><img src="images/W2RMPEQ1nAhx.webp" alt="Trackly" width="114" height="32" loading="lazy"></a>
          <p>The industry-leading automation platform specifical for your business.</p>
          <div class="socials">
            <a href="https://www.facebook.com/profile.php?id=61557669644128" target="_blank" rel="noopener" aria-label="Facebook">
              <svg width="20" height="20" viewBox="0 0 20 21" aria-hidden="true"><path d="M7.584 20.112V13.462H5.522V10.406h2.062V9.09c0-3.405 1.54-4.982 4.882-4.982.334 0 .796.035 1.223.086.32.033.638.087.951.162v2.771a8.6 8.6 0 0 0-.544-.033c-.204-.005-.407-.008-.611-.007-.589 0-1.049.08-1.396.257a1.4 1.4 0 0 0-.566.518c-.215.35-.311.83-.311 1.46v1.081h3.265l-.321 1.753-.239 1.303h-2.705v6.871C16.163 19.735 20 15.519 20 10.406c0-5.522-4.477-10-10-10S0 4.884 0 10.406c0 4.69 3.228 8.625 7.584 9.706Z" fill="currentColor"/></svg>
            </a>
            <a href="https://www.linkedin.com/company/dipainhouse/" target="_blank" rel="noopener" aria-label="LinkedIn">
              <svg width="17" height="16" viewBox="0 0 17 16" aria-hidden="true"><path d="M4 2.537a1.667 1.667 0 1 1-3.333-.002A1.667 1.667 0 0 1 4 2.537ZM4.05 5.437H.717V15.87H4.05V5.437Zm5.267 0H6V15.87h3.283v-5.475c0-3.05 3.975-3.333 3.975 0v5.475h3.292V9.262c0-5.142-5.883-4.95-7.267-2.425V5.437Z" fill="currentColor"/></svg>
            </a>
            <a href="https://x.com/dipa_inhouse" target="_blank" rel="noopener" aria-label="X (Twitter)">
              <svg width="17" height="17" viewBox="0 0 17 17" aria-hidden="true"><path d="M.318.963h4.712l4.193 5.998L14.257.963h1.505L9.797 7.782l6.42 9.181h-4.71L7.127 10.701 1.721 16.963H.217l6.337-7.082L.318.963Z" fill="currentColor"/></svg>
            </a>
          </div>
        </div>

        <nav class="footer-links" aria-label="Footer">
          <a href="{{ route('home') }}">Home</a>
          <a href="{{ route('about') }}">About Us</a>
          <a href="{{ route('services') }}">Services</a>
          <a href="{{ route('case-studies') }}">Case Studies</a>
          <a href="{{ route('blog') }}">Blog</a>
          <a href="{{ route('contact') }}">Contact Us</a>
        </nav>
      </div>

      <hr>

      <div class="footer-bottom">
        <p class="copyright">© 2025 Trackly. All rights reserved.</p>
        <div class="legal">
          <a href="{{ route('privacy-policy') }}">Privacy Policy</a>
          <a href="{{ route('terms-of-service') }}">Terms of Service</a>
        </div>
      </div>
    </div>
  </footer>

  <script src="js/script.js"></script>
</body>
</html>
